...
**B. キャプション + 効果（4DX@HOME向け、Gemini推奨）**
```
必ずJSONオブジェクトで以下の形式を返すこと：
{
  "frames": [
    {"caption": "...", "effects": {"flash": "...", "color": "...", "vibration": "...", "water": "...", "wind": "...", "mist": "..."}}
  ]
}
```
...
`prompts.py` には以下の9つのプロンプトパターンが用意されています：
...
9. `4dx_home_v3` - 4DX@HOME 3世代目（表現力強化＋拡張効果）
