...
#### Gemini版（4DX@HOME仕様）
- **一瞬の風（burst）** - 風を出す（最小1.0秒）

### 💦 水（water）

#### 両バージョン共通
- **水しぶき（burst）** - 水・波・唾・汗の噴射（**shot型: 一度きり**、最小0.5秒）

### 🌫️ ミスト（mist）

#### Gemini版（4DX@HOME仕様）
- **ミスト（burst）** - 霧・蒸気・煙・白い噴霧（**shot型: 一度きり**、最小0.5秒）
...
- **ピンク（pink）** - 感情・暖色（最小2秒）
- **赤（red）** - 炎・火・爆発・血（最小2秒）
- **オレンジ（orange）** - 夕焼け・熱気（最小2秒）
- **黄色（yellow）** - 明るいシーン（最小2秒）
- **黄緑（yellow_green）** - 自然・若葉（最小2秒）
- **緑（green）** - 森・草原・自然（最小2秒）
- **深緑（dark_green）** - 深い森・影（最小2秒）
- **シアン（cyan）** - 水・空（最小2秒）
- **青（blue）** - 空・海・水（最小2秒）
...
```python
PROMPT_NAME = "my_custom_prompt"  # デフォルトは "4dx_home_v3"
```
