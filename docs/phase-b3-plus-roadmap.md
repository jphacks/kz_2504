# 4DX@HOME Phase B-3+ 実装ロードマップ

## 📋 概要
- **作成日**: 2025年10月12日
- **最終更新**: 2025年10月12日 (デバイス検出システム進捗反映)
- **対象フェーズ**: Phase B-3 ～ Phase B-5+
- **前提条件**: Phase B-2 完了 + JSON送信システム完了
- **目標**: フル機能4DX体験システム完成

### 📈 **Phase B-2+ 追加実装状況** (2025年10月12日更新)
- ✅ **JSONファイル送信機能**: サーバ→デバイスハブ通信完成
- ✅ **デバイスハブ稼働想定テスト**: WebSocket送信テスト成功
- ✅ **同期データ処理**: demo1.json (28KB, 185エフェクト) 正常送信
- 🔄 **実デバイス検出移行**: Mock→実WebSocket接続プール準備中

---

## 🎯 Phase B-3: 再生制御API実装

### ⏱️ **実装スケジュール**
- **予定期間**: 2025年10月12日 ～ 2025年10月19日 (7日間)
- **優先度**: **Critical** (システム完成の核心機能)
- **目標**: リアルタイム4D体験の実現

### 🔧 **実装予定機能**

#### **1. 再生制御API Core**
**ファイル**: `app/api/playback_control.py` (新規)

```python
# 実装予定エンドポイント
@router.post("/start/{session_id}")
async def start_playback(session_id: str, playback_request: PlaybackStartRequest):
    """
    再生開始
    - 動画再生開始コマンド
    - 同期タイマー開始
    - デバイスハブ通信開始
    """

@router.post("/sync/{session_id}")  
async def sync_playback(session_id: str, sync_data: PlaybackSyncData):
    """
    リアルタイム同期 (0.1秒間隔)
    - current_time: 現在再生時刻
    - playback_rate: 再生速度
    - effects: 現在のエフェクト指示
    """

@router.post("/pause/{session_id}")
async def pause_playback(session_id: str):
    """一時停止"""

@router.post("/resume/{session_id}") 
async def resume_playback(session_id: str):
    """再生再開"""

@router.post("/stop/{session_id}")
async def stop_playback(session_id: str):
    """再生停止・セッション終了"""

@router.post("/seek/{session_id}")
async def seek_playback(session_id: str, seek_request: SeekRequest):
    """シーク操作"""

@router.get("/status/{session_id}")
async def get_playback_status(session_id: str):
    """再生状態取得"""
```

#### **0. 実デバイス検出システム** 🆕 **優先実装**
**ファイル**: `app/services/device_discovery.py` (新規)

```python
class RealDeviceDiscovery:
    """実デバイス検出サービス"""
    
    async def discover_active_devices(self):
        """
        WebSocket接続プールベースのデバイス発見
        - アクティブWebSocket接続をスキャン
        - デバイス応答性テスト (ping/pong)
        - デバイス能力情報取得
        """
    
    async def monitor_device_connections(self):
        """
        デバイス接続監視
        - 接続状態リアルタイム追跡
        - 接続断絶自動検出
        - デバイスハブ登録・削除管理
        """
    
    async def validate_device_capabilities(self, device_id: str):
        """
        デバイス能力検証
        - サポートアクチュエーター確認
        - パフォーマンス測定
        - 互換性判定
        """
```

#### **2. 同期データ配信システム**
**ファイル**: `app/services/sync_service.py` (新規)

```python
class SyncService:
    """リアルタイム同期管理"""
    
    async def start_sync_loop(self, session_id: str, video_id: str):
        """
        0.1秒間隔での同期データ配信
        - demo1.json から現在時刻の効果データ抽出
        - WebSocket経由でデバイスハブ配信
        - 配信遅延監視・補正
        """
    
    async def process_sync_data(self, current_time: float, video_id: str):
        """
        同期データ処理
        - 現在時刻に対応するエフェクト抽出
        - アクチュエーター命令生成
        - デバイス互換性確認
        """
    
    async def handle_playback_event(self, event: PlaybackEvent):
        """
        再生イベント処理
        - play/pause/seek の状態変更
        - デバイス同期状態更新
        - エラー回復処理
        """
```

#### **3. リアルタイムWebSocket通信**
**ファイル**: `app/api/playback_control.py` (WebSocket追加)

```python
@router.websocket("/ws/sync/{session_id}")
async def sync_websocket(websocket: WebSocket, session_id: str):
    """
    リアルタイム同期WebSocket
    - フロントエンド: 再生時刻送信
    - バックエンド: エフェクト命令送信
    - デバイスハブ: 実行確認応答
    """
```

#### **4. 再生状態管理**
**ファイル**: `app/models/playback.py` (新規)

```python
class PlaybackState(BaseModel):
    session_id: str
    video_id: str
    current_time: float
    playback_rate: float = 1.0
    status: PlaybackStatus  # PLAYING, PAUSED, STOPPED
    started_at: Optional[datetime]
    last_sync_time: float
    device_sync_status: Dict[str, SyncStatus]
    active_effects: List[ActiveEffect]
    sync_offset_ms: float = 0.0

class ActiveEffect(BaseModel):
    actuator_type: ActuatorType
    effect_id: str
    intensity: float
    started_at: float
    duration_ms: int
    status: EffectStatus  # ACTIVE, COMPLETED, FAILED
```

### 📊 **技術仕様・要件**

#### **同期精度要件**
- **目標遅延**: < 100ms (フロントエンド→デバイス)
- **配信間隔**: 100ms (10Hz)
- **時刻精度**: ±50ms
- **復旧時間**: < 5秒

#### **パフォーマンス要件**
- **同時再生セッション**: 50+
- **WebSocket 接続数**: 100+
- **CPU使用率**: < 80%
- **メモリ使用量**: < 2GB

#### **同期データ形式**
```json
{
  "session_id": "session_123",
  "current_time": 15.2,
  "playback_rate": 1.0,
  "timestamp": "2025-10-12T10:30:15.123Z",
  "effects": [
    {
      "actuator": "VIBRATION",
      "action": "start", 
      "intensity": 0.8,
      "duration": 2000,
      "effect_id": "vibration_001"
    },
    {
      "actuator": "WATER",
      "action": "shot",
      "intensity": 0.6,
      "duration": 500,
      "effect_id": "water_burst_001"
    }
  ]
}
```

### 🧪 **Phase B-3 テスト計画**

#### **テストファイル**: `test_phase3_playback.py` (新規作成)

**テスト項目**:
1. **再生制御API基本テスト**
   - 再生開始・停止・一時停止・再開
   - シーク操作
   - エラーハンドリング

2. **リアルタイム同期テスト**
   - 0.1秒間隔配信確認
   - 遅延測定
   - 同期精度検証

3. **WebSocket通信テスト**
   - 双方向リアルタイム通信
   - 接続断絶時復旧
   - 負荷テスト

4. **効果データ配信テスト**
   - demo1.json タイミング精度
   - アクチュエーター命令正確性
   - デバイス互換性

---

## 🎮 Phase B-4: セッション管理拡張

### ⏱️ **実装スケジュール**
- **予定期間**: 2025年10月19日 ～ 2025年10月26日 (7日間)
- **優先度**: **High** (システム安定性・スケーラビリティ)

### 🔧 **実装予定機能**

#### **1. 高度なセッション管理**
**ファイル**: `app/services/session_service.py` (拡張)

```python
class AdvancedSessionManager:
    """高度なセッション管理"""
    
    async def create_session_with_resource_check(self, video_id: str, device_id: str):
        """
        リソース考慮セッション作成
        - デバイス稼働状況確認
        - サーバーリソース確認  
        - セッション優先度設定
        """
    
    async def manage_concurrent_sessions(self):
        """
        並行セッション管理
        - 同時セッション数制限
        - リソース配分最適化
        - セッション優先度調整
        """
    
    async def session_migration(self, session_id: str, target_server: str):
        """
        セッション移行
        - サーバー間セッション移行
        - 状態保存・復元
        - 無停止移行
        """
```

#### **2. リアルタイムデバイス監視**
**ファイル**: `app/services/device_monitor.py` (新規)

```python
class DeviceMonitorService:
    """デバイス状態監視"""
    
    async def monitor_device_health(self, device_id: str):
        """
        デバイスヘルス監視
        - 応答時間監視
        - エラー率追跡
        - パフォーマンス測定
        """
    
    async def predictive_maintenance(self, device_id: str):
        """
        予防保守
        - 異常予測
        - メンテナンス推奨
        - 自動復旧試行
        """
```

#### **3. 負荷分散・スケーリング**
```python
# Cloud Run 自動スケーリング対応
- インスタンス自動増減
- 負荷分散アルゴリズム
- リソース使用量最適化
- レスポンス時間維持
```

---

## 🚀 Phase B-5: 拡張機能・AI統合

### ⏱️ **実装スケジュール**  
- **予定期間**: 2025年10月26日 ～ 2025年11月16日 (3週間)
- **優先度**: **Medium** (差別化機能・将来性)

### 🤖 **AI・機械学習統合**

#### **1. 動的効果最適化**
**ファイル**: `app/services/ai_optimization.py` (新規)

```python
class EffectOptimizationAI:
    """AI効果最適化"""
    
    async def personalize_effects(self, user_id: str, preferences: Dict):
        """
        個人化効果調整
        - ユーザープリファレンス学習
        - 体感強度調整
        - 効果タイミング最適化
        """
    
    async def adaptive_intensity_control(self, session_data: SessionData):
        """
        適応的強度制御
        - リアルタイム体験品質調整
        - デバイス性能適応
        - 環境条件考慮
        """
```

#### **2. 予測的品質管理**
```python
# 機械学習による品質管理
- ネットワーク遅延予測
- デバイス故障予測
- ユーザー満足度予測
- 自動パラメータ調整
```

### 🌐 **クラウド拡張機能**

#### **1. CDN統合**
```python
# 動画コンテンツ配信最適化
- 地域別CDN
- 動的キャッシュ制御
- 帯域幅最適化
- 配信遅延最小化
```

#### **2. 分析・監視基盤**
```python
# 包括的分析システム
- リアルタイムダッシュボード
- ユーザー行動分析
- システムパフォーマンス分析
- ビジネスKPI追跡
```

---

## 🔧 **技術的実装詳細**

### **開発環境セットアップ**

#### **Phase B-3 準備**
```bash
# 新規依存関係追加
pip install asyncio-throttle  # 同期制御
pip install aiortc           # WebRTC (将来用)
pip install numpy           # 数値計算
pip install structlog       # 構造化ログ

# 開発ツール追加
pip install pytest-asyncio   # 非同期テスト
pip install pytest-benchmark # パフォーマンステスト
pip install locust          # 負荷テスト
```

#### **ディレクトリ構造拡張**
```
backend/
├── app/
│   ├── api/
│   │   └── playback_control.py    # 🆕 Phase B-3
│   ├── models/
│   │   └── playback.py            # 🆕 Phase B-3
│   ├── services/
│   │   ├── sync_service.py        # 🆕 Phase B-3
│   │   ├── playback_service.py    # 🆕 Phase B-3
│   │   ├── session_service.py     # 🆕 Phase B-4
│   │   ├── device_monitor.py      # 🆕 Phase B-4
│   │   └── ai_optimization.py     # 🆕 Phase B-5
│   └── websocket/
│       └── sync_handler.py        # 🆕 Phase B-3
├── tests/
│   ├── test_phase3_playback.py    # 🆕 Phase B-3
│   ├── test_phase4_session.py     # 🆕 Phase B-4
│   └── test_integration_e2e.py    # 🆕 統合テスト
└── monitoring/
    ├── dashboards/               # 🆕 Phase B-5
    ├── alerts/                   # 🆕 Phase B-4
    └── analytics/                # 🆕 Phase B-5
```

### **データベース設計** (Phase B-4+)

#### **セッション管理テーブル**
```sql
-- 将来的なPostgreSQL移行計画
CREATE TABLE sessions (
    session_id UUID PRIMARY KEY,
    user_id UUID,
    device_id VARCHAR(100),
    video_id VARCHAR(50),
    status session_status,
    created_at TIMESTAMP,
    started_at TIMESTAMP,
    ended_at TIMESTAMP,
    sync_quality JSONB,
    user_feedback JSONB
);

CREATE TABLE device_metrics (
    device_id VARCHAR(100),
    timestamp TIMESTAMP,
    response_time_ms INTEGER,
    error_rate FLOAT,
    battery_level INTEGER,
    temperature FLOAT,
    PRIMARY KEY (device_id, timestamp)
);
```

### **監視・アラートシステム**

#### **Phase B-4: 基本監視**
```python
# Prometheus メトリクス
- 同期遅延時間
- API レスポンス時間  
- エラー率
- 同時接続数
- リソース使用率
```

#### **Phase B-5: 高度な分析**
```python
# Grafana ダッシュボード
- リアルタイム システム状況
- ユーザー体験品質
- ビジネス KPI
- 予測分析結果
```

---

## 📊 **マイルストーン・成功指標**

### **Phase B-3 成功指標**
- ✅ 同期精度 < 100ms達成
- ✅ 0.1秒間隔配信安定動作
- ✅ 50同時セッション対応
- ✅ demo1.json完全再生成功

### **Phase B-4 成功指標**
- ✅ 100+ 同時セッション対応
- ✅ 99.9%+ システム稼働率
- ✅ 自動スケーリング動作
- ✅ 予防保守アラート機能

### **Phase B-5 成功指標**  
- ✅ AI効果最適化機能
- ✅ CDN統合完了
- ✅ 包括的分析ダッシュボード
- ✅ ユーザー満足度90%+

### **システム全体KPI**
- 🎯 エンドツーエンド遅延: < 300ms
- 🎯 ユーザー離脱率: < 5%
- 🎯 システム可用性: 99.95%+
- 🎯 同期精度: ±50ms以下

---

## ⚠️ **リスク管理・対策**

### **技術的リスク**

#### **Phase B-3 リスク**
- **同期精度課題**: ネットワーク遅延変動
  - **対策**: 適応的遅延補正、バッファリング
- **WebSocket接続安定性**: 接続断絶
  - **対策**: 自動再接続、状態復元

#### **Phase B-4 リスク**
- **スケーラビリティ限界**: リソース不足
  - **対策**: クラウド自動スケール、負荷分散
- **データ整合性**: 分散環境での状態管理
  - **対策**: トランザクション管理、イベントソーシング

#### **Phase B-5 リスク**
- **AI精度**: 機械学習モデルの品質
  - **対策**: A/Bテスト、段階的ロールアウト
- **複雑性増大**: システム保守困難
  - **対策**: マイクロサービス設計、詳細ドキュメント

### **プロジェクトリスク**

#### **スケジュールリスク**
- **遅延要因**: 技術的課題の予期せぬ困難
- **対策**: バッファ期間設定、機能優先度調整

#### **品質リスク**
- **品質低下**: 機能追加による既存機能への影響
- **対策**: 包括的回帰テスト、段階的リリース

---

## 🛣️ **実装戦略・アプローチ**

### **アジャイル開発手法**
- **スプリント期間**: 1週間
- **デイリースタンドアップ**: 毎日
- **スプリントレビュー**: 週1回
- **リトロスペクティブ**: 週1回

### **品質保証戦略**
- **TDD**: テスト駆動開発
- **CI/CD**: 継続的インテグレーション
- **コードレビュー**: 全コミット必須
- **自動テスト**: 80%+ カバレッジ

### **デプロイ戦略**
- **ブルーグリーンデプロイ**: ダウンタイム0
- **カナリアリリース**: 段階的機能展開
- **ロールバック**: 即座復旧可能
- **監視**: リアルタイム品質監視

---

## 📅 **詳細スケジュール**

### **Phase B-3: 2025年10月12日-19日**
```
Day 1-2: 再生制御API基本実装
Day 3-4: 同期サービス実装  
Day 5-6: WebSocket統合・テスト
Day 7: 統合テスト・バグ修正
```

### **Phase B-4: 2025年10月19日-26日**  
```
Day 1-2: セッション管理拡張
Day 3-4: デバイス監視システム
Day 5-6: スケーリング・負荷分散
Day 7: パフォーマンステスト
```

### **Phase B-5: 2025年10月26日-11月16日**
```
Week 1: AI最適化基盤
Week 2: CDN統合・分析システム
Week 3: 総合テスト・最終調整
```

---

**更新責任者**: 開発チーム  
**承認**: Phase B-2 完了レビュー通過  
**次期レビュー**: Phase B-3 完了時