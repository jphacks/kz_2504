...
export const preparationApi = {
  uploadTimeline: async (
    sessionId: string,
    videoId: string,
    timelineData: { events: TimelineEvent[] }
  ) => {
    const payload = {
      video_id: videoId,
      timeline_data: timelineData, // ← { events: [...] } が来る想定
    };
    console.log("[timelineApi] upload payload", payload);
    return apiClient.post<TimelineUploadResponse>(
      `/api/preparation/upload-timeline/${encodeURIComponent(sessionId)}`,
      payload
    );
  },
